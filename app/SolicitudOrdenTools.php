...
class SolicitudOrdenTools
{
  ...
  private function tipo_solicitud($id) {
      ...
      $solicitud = $orden->solicitud()->first();
      if (is_null($solicitud)) { return json_encode($res); }

      $tipo_solicitud = new \stdClass;

        $solicitud_orden = SolicitudOrdenCaptura::find($id);
  ...
  private function solicitudes($id) {
    ...
    $solicitudorden = SolicitudOrdenCaptura::find($id);
  ...
  public function solicitud_orden($solicitud_id, $token){
    $res = new \stdClass;
    $solicitud = SolicitudOrdenCaptura::find($solicitud_id);
    if (is_null($solicitud)) { return json_encode($res); }
    $id = $solicitud->id;
