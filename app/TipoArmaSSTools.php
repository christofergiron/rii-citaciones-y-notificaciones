...
class TipoArmaSSTools
{
  ...
  private function rows($token) {
    ...
    //$res->rows[]=[]; this fails is no data is returned...
      foreach (TipoArmaSS::All() as $tipo_arma) {
        $row = new \stdClass;
        $row->id = $tipo_arma->id;
        $row->descripcion = $tipo_arma->descripcion;
  ...
  //     return $json_result;
  // }
  public function ss_list_tipo_arma($token) {
      $res = new \stdClass;
