...
use App\InformeLogisticoSSTools;
use App\Http\Requests\StoreInformeLogisticoSS;
...
class InformeLogisticoSSController extends Controller
{

    public  $successStatus = 200;
    private $root;
    private $informe_logistico_ss_tools;
    private $StoreInformeLogisticoSS;
    ...
    public function __construct(LoggerInterface $logger)
    {
        ...
        $this->informe_logistico_ss_tools = new InformeLogisticoSSTools;
        $this->root = false;
        $this->StoreInformeLogisticoSS = new StoreInformeLogisticoSS;
    }
    ...
    public function index(Request $request){
      ...
      $arr['token'] = $header;
       ...
       #get denuncia_mp
       $res = $this->informe_logistico_ss_tools->ss_list_informes($arr["token"]);
    ...
    public function show(Request $request, $id) {
      ...
      $this->log::alert($arr);
       ...
       #get denuncia_mp
       $res = $this->informe_logistico_ss_tools->get_informe($id, $arr["token"]);
    ...
    public function store(Request $request) {
      ...
      $this->logger->alert('inside Store Informe Logistico');
      $this->log::alert(json_encode($arr));

      $res = $this->StoreInformeLogisticoSS->rules($arr);

      if ($res->code != 200) {
        return response()->json(['error'=>$res->message], 403);
      }

      $res = $this->StoreInformeLogisticoSS->persist($arr);
    ...
    public function workflow(Request $request) {
      ...
      $res = $this->StoreInformeLogisticoSS->workflow_rules($arr);
      ...
      $res = $this->StoreInformeLogisticoSS->apply_transition($arr);
