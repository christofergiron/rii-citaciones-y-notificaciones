...
use Validator;
use App\Passport;
class InformeDelitoContraVidaSSTools
{
  private $ss_require;
    private $log;
  ...
  private function acciones($token, $informe_delito_contra_vida_ss) {
      $acciones = [];
      $user_email = $this->get_email_from_token($token);
      if (!isset($user_email)) { return $acciones; }
      if (empty($user_email)) {return $acciones; }
      $acciones = $this->workflow_actions($informe_delito_contra_vida_ss, $user_email);
      $acciones[] = 'Informe';
      return $acciones;
  }
  ...
  private function rows($token) {
    ...
    $iteracion = Informe::where('tipoable_type','App\InformeDelitoContraVidaSS')->get();
