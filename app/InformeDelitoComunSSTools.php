...
use Validator;
use App\Passport;
class InformeDelitoComunSSTools
{
  private $ss_require;
    private $log;
  ...
  private function acciones($token, $informe_delito_comun_ss) {
      $acciones = [];
      $user_email = $this->get_email_from_token($token);
      if (!isset($user_email)) { return $acciones; }
      if (empty($user_email)) {return $acciones; }
      $acciones = $this->workflow_actions($informe_delito_comun_ss, $user_email);
      $acciones[] = 'Informe';
      return $acciones;
  }
  ...
  private function rows($token) {
    ...
    $iteracion = Informe::where('tipoable_type','App\InformeDelitoComunSS')->get();
  ...
  private function obtener_informe($informe, $token) {
      ...
      $item->informe_delito_comun_ss = $informe->tipoable()->get();
