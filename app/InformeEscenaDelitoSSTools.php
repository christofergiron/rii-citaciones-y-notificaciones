...
use Validator;
use App\Passport;
class InformeEscenaDelitoSSTools
{
  private $ss_require;
    private $log;
  ...
  private function acciones($token, $informe_escena_delito_ss) {
      $acciones = [];
      $user_email = $this->get_email_from_token($token);
      if (!isset($user_email)) { return $acciones; }
      if (empty($user_email)) {return $acciones; }
      $acciones = $this->workflow_actions($informe_escena_delito_ss, $user_email);
      $acciones[] = 'Informe';
      return $acciones;
  }
  ...
  private function rows($token) {
    ...
    $iteracion = Informe::where('tipoable_type','App\InformeEscenaDelitoSS')->get();
  ...
  private function obtener_informe($informe, $token) {
      ...
      $item->informe_escena_delito_ss = $informe->tipoable()->get();
